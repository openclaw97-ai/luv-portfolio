...
export default function YouTubeDownloader() {
  ...
  const handleDownload = () => {
    setError('Backend not connected. This is a UI preview.');
  };
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 py-12 pt-32">
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-4">
            YouTube <span className="text-accent">Downloader</span>
          </h1>
          <p className="text-accent-muted max-w-xl">
            Download videos and audio from YouTube. Supports MP4 and MP3 formats.
          </p>
        </motion.div>
        ...
        >
          <form onSubmit={handleFetch} className="space-y-6">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-3 border transition-all ${
                    format === 'mp4'
                      ...
                      : 'border-grid-strong text-accent-muted hover:border-accent/50'
                  }`}
                >
            ...
            >
              {loading ? (
                ...
                <><Download className="w-4 h-4" /> Fetch Info</>
              )}
            </button>
          </form>
          ...
          {error && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="mt-6 p-4 border border-red-500/30 bg-red-500/10 text-red-400 text-sm"
            >
              {error}
            </motion.div>
          )}
          ...
          {videoInfo && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="mt-8 p-6 border border-grid-strong bg-neutral-900/40"
            >
              <div className="flex gap-6 flex-col md:flex-row">
                <img
                  src={videoInfo.thumbnail}
                  alt={videoInfo.title}
                  className="w-full md:w-64 aspect-video object-cover border border-grid-strong"
                />
                <div className="flex-1 space-y-3">
                  ...
                  <p className="text-sm text-accent-muted">Duration: {videoInfo.duration}</p>
                  <p className="text-sm text-accent-muted">Format: {format.toUpperCase()}</p>
                  
                  <button
                    onClick={handleDownload}
                    className="inline-flex items-center gap-2 bg-accent text-background px-6 py-3 font-mono font-bold text-xs tracking-widest hover:bg-accent-muted transition-all mt-4"
                  >
        ...
        <motion.div
          ...
          className="mt-16 p-4 border border-yellow-500/20 bg-yellow-500/10 text-yellow-400/80 text-sm max-w-2xl"
        >
          <strong>Note:</strong> Backend not yet connected. This is a UI preview. Requires yt-dlp integration or external API for actual downloads.
        </motion.div>
      </main>
    </div>
  );
